...
import sys
# release-trigger: 2026-09-18 v5.2.9
...
xmc_scan_app_backups() {
  ...
  for root in /Applications "$HOME/Applications"; do
    ...
    for p in "$root"/*.backup.*.app(N) "$root"/*_backup_*.app(N) "$root"/*.bak.*.app(N); do
      [[ -d "$p" ]] || continue
      [[ "$p" == "$APP_ROOT" ]] && continue
      name="${p:t}"; stem="${name%%.backup.*}.app"
      [[ "$stem" == "$name" ]] && stem="${name%%_backup_*}.app"
      live="$root/$stem"; [[ -d "$live" ]] || continue
  ...
  local c kb; c=$(/usr/bin/awk 'END{print NR+0}' "$out"); kb=$(/usr/bin/awk -F '\t' '{s+=$3}END{print s+0}' "$out")
  echo "APP_BACKUP_COUNT=$c"; echo "APP_BACKUP_KB=$kb"; echo "APP_BACKUP_RESULT=$out"
}
xmc_clean_app_backups() {
  ...
  local trash="$HOME/.Trash" p kb total=0 count=0 base dst n; /bin/mkdir -p "$trash"
  while IFS=$'\t' read -r _ _ kb _ p; do
    [[ -d "$p" ]] || continue; [[ "$p" == "$APP_ROOT" ]] && continue
  ...
  echo "APP_BACKUP_REMOVED_COUNT=$count"; echo "APP_BACKUP_REMOVED_KB=$total"
}
'''
